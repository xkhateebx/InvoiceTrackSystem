...
public class InvoiceCriteriaRepository {
    ...
    private Predicate getPredicate(InvoiceSearchCriteria invoiceSearchCriteria,
        ...
        if ((invoiceSearchCriteria.getOwner().getUserRoles().get(0).getRole().getRoleName()).equals("ADMIN") || (invoiceSearchCriteria.getOwner().getUserRoles().get(0).getRole().getRoleName()).equals("AUDITOR"))
        {
            return criteriaBuilder.and(predicates.toArray(new Predicate[0]));

        }
        else if (Objects.nonNull(invoiceSearchCriteria.getOwner())) {
            predicates.add(
